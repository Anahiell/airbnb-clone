...
namespace Airbnb.TagManagement.API.Controllers;
    ...
    public class TagController(IMediator mediator) : ControllerBase
    {
        ...
        [HttpGet]
        [Route("GetAllTags")]
        [SwaggerOperation(Summary = "Получить теги", Description = "Получает теги из базы данных с пагинацией и фильтрацией")]
        public async Task<IActionResult> GetAllTagsAsync([FromQuery] GetTagPaginatedQuery query, CancellationToken cancellationToken)
        {
        ...
        [HttpGet]
        [Route("GetTagById")]
        [SwaggerOperation(Summary = "Получить тег по Id", Description = "Получает тег из базы данных по его идентификатору")]
        public async Task<IActionResult> GetTagByIdAsync([FromQuery] GetTagPaginatedQuery query, CancellationToken cancellationToken)
        {
            var result = await mediator.Send(query, cancellationToken);
            return Ok(result.Value);
        }
        ...
        public async Task<IActionResult> UpdateTagAsync([FromBody] UpdateTagCommand command, CancellationToken cancellationToken)
        {
            var result = await mediator.Send(command, cancellationToken);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> DeleteTagAsync([FromBody] DeleteTagCommand command, CancellationToken cancellationToken)
        {
            var result = await mediator.Send(command, cancellationToken);
            return Ok(result);
        }
